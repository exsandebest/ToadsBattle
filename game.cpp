...
Game::Game(QWidget *parent, int lvl) : QDialog(parent), ui(new Ui::Game), botLevel(lvl) {
    ...
    for (int i = 0; i < fieldSize; ++i){
        for (int j = 0; j < fieldSize; ++j){
            ...
            connect(btn, SIGNAL(clicked()), this, SLOT(btnGameClicked()));
            field[i][j] = btn;
    ...
    int protectedCellCount = QRandomGenerator::global()->generate() % (maxProtectedCellCount + 1);
    for (int i = 0; i < protectedCellCount; ++i) {
       setProtectedCell(QPoint((QRandomGenerator::global()->generate() + 1) % 7,
                               (QRandomGenerator::global()->generate() + 1) % 7));
    }
}
...
void Game::btnGameClicked() {
    ...
    if (selectionState == 0) {
        ...
        step botStep = bot->nextStep(fieldToNum());
...
bool Game::checkEnd() {
    ...
    QApplication::processEvents();
    End *endWindow = new End(this, gameResult);
    QObject::connect(endWindow, &End::end, this, &Game::close);
    endWindow->open();
    return true;
}

void Game::on_btn_skip_clicked() { skipClicked = true; }
